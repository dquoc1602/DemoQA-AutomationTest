...
import locator.demoqa.elements.CheckboxLocators;
import model.enums.CheckState;
import model.enums.TreeResolvePolicy;
...
import static common.Constants.DEMOQA_CHECKBOX_URL;
import static common.Constants.TREE_POLLING_INTERVAL;
import static common.Constants.TREE_STABILIZATION_TIMEOUT;
...
public class CheckboxPage extends BasePage {
    ...
    public CheckboxPage selectByPath(String... path) {
        ...
        WebElement node = resolveNode(path, TreeResolvePolicy.EXPAND_PARENTS_ONLY);
    ...
    public CheckState getNodeState(String... path) {
        WebElement node = resolveNode(path, TreeResolvePolicy.NO_EXPAND);
    ...
    public int getChildrenCount(String... path) {
        WebElement node = resolveNode(path, TreeResolvePolicy.EXPAND_SELF);
    ...
    private WebElement resolveNode(String[] path, TreeResolvePolicy policy) {
    ...
    private Optional<WebElement> tryResolveNode(String... path) {
        return tryResolveNode(path, TreeResolvePolicy.NO_EXPAND);
    }

    private Optional<WebElement> tryResolveNode(String[] path, TreeResolvePolicy policy) {
        ...
        for (int i = 0; i < path.length; i++) {
            ...
            // if (!isLast || policy == TreeResolvePolicy.EXPAND_SELF) {
